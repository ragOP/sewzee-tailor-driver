...
import { get, patch } from '../api/apiHelper';
import { ToastMsg } from '../Ui/ToastMsg';

const ParticularOrderDetails = ({ navigation ,route}) => {
  const { orderData } = route.params;
  const [isLoading, setIsLoading] = useState(false);
  const [orderDetails, setOrderDetails] = useState(null);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [progressDays, setProgressDays] = useState(50);
  const boxArray = ['Box 1', 'Box 2', 'Box 3', 'Box 4', 'Box 5']; 
  ...
  const fetchOrderDetails = async () => {
    try {
      setIsLoading(true);
      const response = await get(endpoints.TAILOR_GET_ORDER_BY_ID + orderData.id);
      if (response) {
  ...
  const handleAccept = () => {
    ...
    patch(endpoints.TAILOR_GET_ORDER_BY_ID + orderData.id, dataToUpdate)
      .then((response) => {
        // navigation.navigate('DriverMainPage');
        // console.log('Response from Accepted PATCH API:', response);
  ...
  const handleReject = () => {
    const dataToUpdate = { status: 'rejected' };
    patch(endpoints.TAILOR_GET_ORDER_BY_ID + orderData.id , dataToUpdate)
      .then((response) => {
        // navigation.navigate('DriverMainPage');
        ToastMsg(response?.message, 'success')
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.middleContainer}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Text style={styles.headingText}>{'Order No.- ' + orderDetails?.id}</Text>
          <View style={[styles.verticalBorder, { height: '80%' }]} />
          <Text style={styles.headingText}>{orderDetails?.category}</Text>
        </View>
        ...
        <View style={{ paddingHorizontal: 10, marginVertical: 10 , justifyContent: 'space-between', flexDirection: 'row', paddingRight: 15, alignItems: 'center'}}>
          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            <Text style={styles.labelTxt}>{orderDetails?.order_for[0].name}</Text>
          </View>
          <View style={{ flexDirection: 'row' }}>
            <Text style={styles.labelTxt}>Address - </Text>
            <Text style={styles.labelTxt}>{orderDetails?.address?.address + ", " + orderDetails?.address.city }</Text>
          </View>
          <View style={{ flexDirection: 'row' }}>
            <Text style={styles.labelTxt}>Due on - </Text>
            <Text style={styles.labelTxt}>{orderDetails?.pickup_date}</Text>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View style={{ flexDirection: 'row' }}>
              <Text style={styles.labelTxt}>Amount - </Text>
              <Text style={styles.greenTxt}>{"₹" + orderDetails?.tailorPrice}</Text>
...
/>
        ...
        <View style={{ marginTop: 10, paddingHorizontal: 10 }}>
          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            <Text style={[styles.labelTxt, {fontSize: 16}]}>{orderDetails?.order_for[0].name}</Text>
          </View>
        </View>
      ...
      <View style={styles.bottomContainer}>
        <TouchableOpacity style={[styles.button, { borderColor: AppColor.green }]} onPress={handleAccept}>
