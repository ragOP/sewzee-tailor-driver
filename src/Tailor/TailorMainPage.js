...
const TailorMainPage = ({ navigation }) => {
  ...
  const methodClockIn = async () => {
    try {
      setIsLoading(true)
      const response = await get(endpoints.TAILOR_CLOCK_IN);
      if (response) {
  ...
  const methodClockOut = async () => {
    try {
      setIsLoading(true)
      const response = await get(endpoints.TAILOR_CLOCK_OUT);
      if (response) {
        ToastMsg(response?.message, 'success');
      }
    } catch (error) {
  ...
  const methodGetActiveHours = async () => {
    try {
      const response = await get(endpoints.TAILOR_ACTIVE_HOURS);
      if (response) {
